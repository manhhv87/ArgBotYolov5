...
class LoadStreams:
    def __init__(self, sources="file.streams", img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        cameraMatrix = np.array([[797.4981, 0, 323.1862],
                                 [0, 797.8648, 263.6120],
                                 [0, 0, 1]])
        distCoeffs = np.array([0.0202, 0.2425, 0.0053, -3.4086e-04, 0])
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            # h_new, w_new = self.imgs[i].shape[:2]
            # new_camera_matrix, roi = cv2.getOptimalNewCameraMatrix(cameraMatrix, distCoeffs, (w_new, h_new), 1, (w_new, h_new))

            def undistort_frame(frame):
                # return cv2.undistort(frame, cameraMatrix, distCoeffs, None, new_camera_matrix)
                return cv2.undistort(frame, cameraMatrix, distCoeffs)

            self.threads[i] = Thread(target=self.update, args=([i, cap, s, undistort_frame]), daemon=True)
    ...
    def update(self, i, cap, stream, undistort_frame):
